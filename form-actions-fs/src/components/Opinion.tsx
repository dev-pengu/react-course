...
import type { Opinion as OpinionType } from "../types";
import { OpinionsContext } from "../store/opinions-context";

export function Opinion({ opinion: { id, title, body, userName, votes } }: {opinion: OpinionType}) {

  const { upvoteOpinion, downvoteOpinion } = use(OpinionsContext);

  const [ optimisticVotes, setVotesOptimistically ] = useOptimistic<number, 'up' | 'down'>(votes, (prevVotes, mode) => mode === 'up' ? prevVotes + 1 : prevVotes - 1);
